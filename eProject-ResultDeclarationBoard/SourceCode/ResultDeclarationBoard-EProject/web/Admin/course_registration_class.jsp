...
<%
    if (request.getSession().getAttribute("login") == null) {
        response.sendRedirect("login.jsp?returnUrl=course_registration_class.jsp");
        } else if (request.getSession().getAttribute("lockscreen") != "UN") {
        response.sendRedirect("lock_screen.jsp?returnUrl=course_registration_class.jsp");
    } else {%>
...
<html lang="en">
    <head>
        ...
        <title>Student_Subject Manage - Add class</title>
                                        ...
                                        <div class="col-sm-10">
                                            <select class="form-control" name="class">
                                                <c:forEach var="row" items="${class}"> 
                                                    <option value="${row.cla_Id}"><c:out value="${row.cla_Id}"/>- <c:out value="${row.cla_Name}"/></option>
                                                </c:forEach>
                                            </select>                                  
                                        </div>
                                        <br/><br/>
                                        <input type="submit" class="btn btn-success btn-lg" data-toggle="modal" data-target="#myModal" name="btn" value="Registers" />
                                    </table>
