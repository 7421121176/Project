...
<%
    if (request.getSession().getAttribute("login") == null) {
        response.sendRedirect("login.jsp?returnUrl=branch.jsp");
    } else if (request.getSession().getAttribute("lockscreen") != "UN") {
        response.sendRedirect("lock_screen.jsp?returnUrl=branch.jsp");
    } else {%>
...
<html lang="en">
    <head>
        ...
        <title>KINGSTONE ACADEMIC - Branch Manage</title>
        ...
        *********************************************************************************************************************************************************** -->
        ...
        <section id="main-content">
            <section class="wrapper">
                <h3><i class="fa fa-angle-right"></i> Branch Manage </h3>

                <div class="row mt">
                    <div class="col-md-12">
                        <div class="content-panel">
                            <table class="table table-striped table-advance table-hover">
                                <form action="branchA.jsp">
                                        ...
                                        <c:forEach var="row" items="${branch}">
                                            <pg:item> 
                                                <tr>
                                                    ...
                                                    <td><c:out value="${row.bra_Address}"/></td> 
                                                    <td><c:out value="${row.bra_Email}"/></td> 
                                                    <td><c:out value="${row.bra_Phone}"/></td>
                                                    <c:url var="edit" value="../CBranch">
                                                        <c:param name="btn" value="Edit" />
                                                        <c:param name="ebranchID" value="${row.bra_Id}" />
                                                        <c:param name="ebranchname" value="${row.bra_Name}" />
                                                        <c:param name="eaddress" value="${row.bra_Address}" />
                                                        <c:param name="eemail" value="${row.bra_Email}" />
                                                        <c:param name="ephoneno" value="${row.bra_Phone}" />
                                                        <c:param name="p" value="${param.pageNum}"/> 
                                                    </c:url> 
                                                    <c:url var="delete" value="../CBranch">
                                                        <c:param name="btn" value="Delete" />
                                                        <c:param name="braid" value="${row.bra_Id}" />                                                
                                                        <c:param name="p" value="${param.pageNum}"/> 
                                                    </c:url>
                                                    <td>
